...
#
# This example is to show how to restart from an already fininshed run.
# CDNest save the snapshot of runs every 0.2*max_num_saves into files named "restart_dnest.txt_*****"
# we can restart from these saved snapshot
# 
#  1) First run the example rastrigin.py, namely,
#      mpiexec -n np python rastrigin.py
#  2) Then run this example rastrigin_restart.py.
...
# restart sampler from 10000 steps (last step of prevous run)
# one can reset "max_num_saves", "max_num_levels", or "ptol"
logz = sampler.restart(restart_file="restart_dnest.txt_10000", max_num_saves=15000, max_num_levels = 20)
comm.Barrier()
