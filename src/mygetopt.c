                              ...
                              const struct my_option *longopts, int *longindex, int long_only)
{
  ...
  /* advance scan of next option block and permute argv 
   * for option like '-abcd', need to handle each option character and optnext!=NULL 
   * after all charactors are handled, optnext=NULL and permute non-option arguments if needed
   */
  if (my_optnext == NULL && start != 0) 
  {
    ...
    /* permution is done, no need to do again until start is nonzero,
     * namely, a non-option argument is found. */
    start = 0; 
  }
  ...
  if (my_optnext == NULL) 
  {
    /* for an option, the first charactor is '-', so skip it */
...
static int my_getopt_shortopts(int argc, char * const argv[], const char *optstring, int long_only)
{
  ...
  /* opt is not in optstring, return '?' and set my_optnext = NULL */
  if (os == NULL)
  {
    my_optarg = NULL;
    if (long_only) 
  ...
  else /* if not have an argument */
  {
    ...
    if (my_optnext[1] == 0) 
    {
      /* options are input in separate block, like '-a -b -c -d' */
      my_optnext = NULL;
      my_optind++;
    } 
    else 
    {
      /* point to next option charactor. options are input in block, like '-abcd' */
      my_optnext++;
    }
  }
  return opt;
